...
class ControlBar:
    def __init__(self, master, prompter):
        ...
        self.win.geometry("820x90+100+310")
    ...
    def _build(self):
        ...
        tk.Checkbutton(row1, text="📌 On Top", variable=self.ontop_var,
                       command=self._toggle_ontop,
                       bg="#111", fg="#aaa", selectcolor="#333",
                       font=("Helvetica Neue", 12), relief=tk.FLAT,
                       activebackground="#111").pack(side=tk.LEFT, padx=4)
        ...
        # Opacity slider
        tk.Label(row2, text="Opacity", bg="#111", fg="#666",
                 font=("Helvetica Neue", 11)).pack(side=tk.LEFT, padx=(14, 0))
        self.opacity_slider = tk.Scale(
            row2, from_=0.1, to=1.0, resolution=0.05, orient=tk.HORIZONTAL,
            command=self._set_opacity,
            bg="#111", fg="#aaa", highlightthickness=0,
            troughcolor="#222", activebackground="#00e5ff",
            length=110, sliderlength=16, width=10,
        )
        self.opacity_slider.set(0.94)   # matches PrompterWindow default
        self.opacity_slider.pack(side=tk.LEFT, padx=(4, 0))
    ...
    def _set_opacity(self, val):
        self.prompter.win.attributes("-alpha", float(val))

    def _toggle_ontop(self):
        state = self.ontop_var.get()
        self.prompter.win.attributes("-topmost", state)
